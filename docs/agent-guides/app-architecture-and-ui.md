...
当前布局常量为水平外边距 `22pt`、边栏顶部边距 `20pt`、边栏底部边距 `21pt`、边栏宽度 `272pt`、设置内容最大宽度 `760pt`、窗口圆角 `26pt`，最大内容宽度为 `22 + 272 + 22 + 760 + 22 = 1098pt`。调整任一常量时必须同步检查 `MainView.swift` 与 `NormalLauncher.swift`：左右窗口边距、边栏到内容距离和内容到右边界距离保持 `22pt`，边栏顶、底边距分别保持 `20pt`、`21pt`。由于窗口采用透明标题栏，`NormalLauncher` 创建 `NSHostingController` 后必须将其 `safeAreaRegions` 设为空，使 SwiftUI 根视图不再收到标题栏安全区；`MainView` 随后以普通的 `HStack` 顶部 `20pt` padding 与侧栏底部 `21pt` padding 定义边距，不得用 `.ignoresSafeArea()` 或无界 frame 在 SwiftUI 内补偿。背景单独忽略安全区只负责铺满视觉层。最低部署目标为 macOS 14.0；不得引入 macOS 26 专属 API。低版本出现轻微安全区或最大宽度偏差时，优先调整常量或 `window.maxSize`，不要重写窗口架构。
