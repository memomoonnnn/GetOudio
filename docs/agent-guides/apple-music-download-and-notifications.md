...
适用于 Apple Music 下载、JSONL 事件、常驻 Agent、通知派发、通知授权和通知队列。修改前检查 `GetOudioAMRuntimeAgent/Sources/`、`AppleMusicDownloadService`、进度解析、`NotificationEventQueue`、`NotificationService`、`NotificationDispatchWaker`、`NotificationAuthorizationModel` 和 Share 下载协调逻辑。
...
完成或失败事件（包括录音、转换和 Apple Music 下载）只能进入 App Group `NotificationEventQueue`，再由 `NotificationDispatchWaker` 唤醒 `HeadlessRunner`。`NotificationService.dispatchPendingNotificationEvents()` 是唯一可认领、渲染、提交和确认删除的入口：仅 `UNUserNotificationCenter.add` 成功后确认；提交失败按 2、10、60 秒重试，授权拒绝或耗尽重试后转入 `suppressed`，不得在以后补发过期完成通知。改动协议至少覆盖旧转换事件兼容解码、写入、claim-by-move、确认、重试、抑制、超时回队与重复 wake 幂等性，并检查 `notification event enqueue`、`notification event claim`、`notification scheduled` 和 `notification dispatch wake requested` 日志。
