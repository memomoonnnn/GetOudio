...
关于各个功能的使用教程，请切换左侧边栏，在不同的设置页中一一查看。

另外，这个设置窗口的背景是透明的，最好不要把窗口放在纯黑色背景上，这会使GUI变得难以辨别。
...
- 完成依赖安装和初始化后，「Get Oudio」将在你的电脑上占用超过1GB的空间，这是因为源项目是通过在一个虚拟的Android容器中靠Wrapper爬取数据实现的，「Get Oudio」需要搭一个虚拟机，目前采用的Colima+Docker CLI已经是体积最小的方案了。
