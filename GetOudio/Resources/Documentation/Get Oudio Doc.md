...
过去类似Permute、Downie这样的软件，你需要先打开窗口、拖放文件、设定参数再启动进程，完成任务后还要手动退出，这种交互设计在我看来是非常愚蠢的。于是「Get Oudio」参考了一众解压缩软件的设计逻辑，将操作入口集成在MacOS本身的组件中，全程没有任何窗口干扰，只有右上角的横幅通知告知你进程状态，就好像Mac天生就自带这个功能一般。
