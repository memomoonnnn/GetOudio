...
The Apple Music wrapper is managed as a Docker image, not as an embedded macOS executable. The same-bundle persistent Get Oudio background agent installs Docker CLI, Colima, Lima, and GPAC under the app's sandbox-local Application Support `GetOudioV2` root, uses `ghcr.io/itouakirai/wrapper:arm` as `linux/arm64` on Apple Silicon and `ghcr.io/itouakirai/wrapper:x86` as `linux/amd64` on Intel, and stores mutable wrapper data under that runtime root. GPAC defaults to the official macOS package and extracts its relocatable `GPAC.app/Contents/MacOS` runtime; `GET_OUDIO_GPAC_PACKAGE_URL` is an optional override passed from the main app to the agent. This is a fresh-install data layout and does not read, migrate, or remove legacy App Group data.
